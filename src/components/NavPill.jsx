...
//
// Narrow the dock shares its line with the search bar, so the pill gives up its
// arrows — the width of two more buttons is the difference between a field you
// can read what you typed in and a stub. Nothing is lost by it: a swipe across
// the page turns the chapter (see lib/swipe.js), and the contents marker in the
// top corner opens the whole book at once, which the arrows could never do.
//
...
export default function NavPill({ trail = [], chapter, atStart, atEnd, onPrev, onNext, onBack, search }) {
  return (
    <nav className="navdock">
      <div className="navpill" style={{ ...glassPill, display: "flex", alignItems: "center", gap: 4, padding: 6, pointerEvents: "auto" }}>
